...
\title{Gate-Controlled Admission Ordering for Burst-Submitted Kubernetes Training Jobs: Pilot Evidence and a Synthetic Scaling Projection}
...
\IEEEcompsocthanksitem Majid Bafandeh: \href{mailto:majid.bafandeh@aut.ac.ir}{majid.bafandeh@aut.ac.ir}.}}

\markboth{Manuscript prepared for journal review, 2026}{Mirzaei Movahed and Bafandeh: Gate-Controlled Admission Ordering}
\maketitle

\begin{abstract}
Kubernetes makes mature Pod-placement decisions, but its default policy does not derive a burst-relative execution order from training characteristics. We present \system, a gate-controlled admission layer that preserves the default scheduler's feasibility, scoring, and binding path. The six-feature ranking ratios are adopted from Abdelshaheed and Ashour; our contribution is their validated Kubernetes-native realization through Scheduling Readiness gates, auditable application-start evidence, a telemetry-bounded FastPath, and bounded tail balancing. We implemented the controller as an in-cluster service and executed real Kubernetes Jobs on a shared four-core K3s node. Five paired repetitions compared 12 controlled NumPy training jobs under synchronized FIFO admission and the deployed policy. Mean job completion time fell from 31.047 to 28.755~s, a mean paired improvement of 7.31\% (exploratory 95\% CI [2.57\%, 12.05\%]). Makespan changed from 55.369 to 54.005~s (2.47\%); its paired interval includes zero. Separately, we report an explicitly optimistic synthetic what-if for 200 generated heavy-training workloads. Under the declared assumption that the complete policy improves on duration-only ordering by a further 6\% in average JCT, 3\% in p95 JCT, and 2\% in makespan, the projection is 30.0\%, 13.7\%, and 12.8\% better than the Kubernetes-default proxy, respectively. These 200-workload values are scenario outputs, not executed Kubernetes measurements. The evidence therefore establishes an auditable production path and a bounded scaling hypothesis, not general superiority to duration-only scheduling or performance for GPU and multi-node training.
\end{abstract}

\begin{IEEEkeywords}
Kubernetes, scheduling readiness, admission ordering, training jobs, FastPath, job completion time.
\end{IEEEkeywords}
...
Kubernetes exposes a rich scheduling framework for deciding \emph{where} a Pod may run. The default scheduler filters infeasible nodes, scores the remaining candidates, and binds each Pod while respecting requests, taints, affinity, storage, and topology constraints~\cite{kubernetesSchedulingFramework}. It does not, however, derive a burst-relative priority from training behavior. A PriorityClass maps a cluster-scoped name to a fixed integer and places higher-priority eligible Pods earlier in the scheduler queue; unless configured as non-preempting, it may also evict lower-priority work~\cite{kubernetesPriority}. It is useful for operator-declared service importance, but it does not compute a score relative to the other jobs in one burst. \system{} instead recomputes ordering after complete-burst validation and uses one-way gates without changing Pod priority or invoking preemption. An operator could manually translate the same features into PriorityClasses, but that would be a separate static policy and would require managing a finite global class set rather than a burst-local order.
...
The paper makes three system contributions:
\begin{itemize}
  \item a Kubernetes-native controller that applies an adopted, deterministic training rank through scheduling gates while retaining \texttt{default-scheduler};
  \item a fail-closed FastPath and bounded tail balancer whose decisions, inputs, and application-level execution evidence are persisted for replay and audit; and
  \item a paired real-cluster pilot plus an artifact-backed structural audit that exposes both the observed benefit and the unresolved duration-only confound.
\end{itemize}

The evidence is deliberately bounded. The workload is a controlled training proxy rather than a neural-network benchmark; the cluster has one shared CPU node; and the five repetitions are insufficient for broad claims about other workloads or environments. The 200-workload scenario is additionally synthetic and optimistic by construction. The aim is to establish an auditable production path, a credible first measurement, and a transparent scaling hypothesis---not to claim an optimal general-purpose ML scheduler.
...
The policy also has a classical scheduling interpretation. Favoring smaller $T$ is an SPT/weighted-SJF-style heuristic related to Smith's sequencing rule~\cite{smith1956optimizers}; assigning a ranked list to a bounded number of predicted lanes is list scheduling in the sense studied by Graham~\cite{graham1969bounds}. The six-feature score augments the duration term, while the tail balancer performs a bounded exhaustive search over at most four jobs. We therefore do not claim a new scheduling primitive or optimality result. The novelty evaluated here is the default-scheduler-preserving gate-control path and its evidence model.
...
\label{eq:rank}
\end{equation}
Jobs are sorted by decreasing $S_j$. The relative coefficients are adopted from~\cite{abdelshaheed2025intelligent} and uniformly divided by 1.25, changing the original sum from 1.25 to 1 while preserving every score ratio and order. They were not tuned on the pilot. In the current generator, $T$ is itself derived from $R,M,G,C,$ and $P$, so the six inputs are not independent. The score is therefore an explicit weighted-SPT-like policy, not a learned predictor or an optimality proof. Scores are local to one burst and cannot be compared across runs.

We quantified this dependence from the 60 archived job manifests (five bursts of 12). Within each burst, the six-feature score and the duration-only desirability score had mean Pearson correlation 0.944, Spearman $\rho=0.922$, and Kendall $\tau_b=0.818$. The full order never matched exactly: 6.8 jobs changed position per burst, the mean absolute shift was 0.9 places, and the top-three overlap was 2.6 of 3. Thus the additional terms do change decisions, but the high correlation confirms that the current pilot cannot attribute its measured JCT change specifically to information beyond $T$. These are structural diagnostics, not a live duration-only performance result.
...
\label{eq:headroom}
\end{equation}
If the whole burst fits, all gates are removed in ranked order. If at least one ranked Pod fits but the burst exceeds current headroom, the implementation still prefills the complete ranked queue; the default scheduler then keeps infeasible Pods pending according to its normal requests and constraints. If telemetry is stale or no bounded demand can be established, FastPath fails closed and the controller returns to the conservative path. In all five reported runs, $D=12$ cores, $A=4$ cores, and mean $u=0.304$, yielding mean $u_{\mathrm{projected}}=3.304>\tau$. Consequently, the whole-burst-fit branch was never selected: every run used \texttt{ranked\_queue\_prefill}, with a two-Pod headroom estimate and 12 initial gate removals. This is recorded behavior, not an inference from configuration.

Before queue prefill, the controller may permute at most the four lowest-ranked jobs. It predicts a list schedule from $T$, keeps the high-priority prefix fixed, and chooses the suffix with the lowest predicted makespan, using cumulative completion time as a secondary criterion. With $N=12$ and window four, the recorded protected prefix is eight jobs; tail balancing was selected in all five runs. At most $4! = 24$ permutations are examined. Validation and feature normalization are $O(N)$, ranking is $O(N\log N)$, and the bounded suffix search does not change the asymptotic complexity.

\section{Workload and Experimental Method}

\subsection{Controlled training proxy}

The experiment uses work-model version 2.0 and a containerized NumPy trainer~\cite{harris2020numpy}. For each step, the process multiplies partitions of two $M\times M$ float matrices, updates a $G$-MiB gradient buffer, and performs $P-1$ in-memory buffer copies when $P>1$. Synthetic loss follows
\begin{equation}
L_{s+1}=L_s e^{-R},
\end{equation}
and training stops below 0.02 or at a bounded step budget. Specifically, convergence steps are $\lfloor\log(1/0.02)/R\rfloor+1$ and the budget is $\min(512,\max(64,\lceil4(M/32)(1+G/10)\rceil))$. The planned step count is their minimum. Every $C$ steps, the process writes and \texttt{fsync}s a payload between 4~KiB and 16~MiB. The estimator uses 0.5~ms for a $128^2$ matrix operation, 5000~MiB/s for gradient copies, 250~MiB/s for checkpoints, and 0.5~ms per partition synchronization. These constants are explicit reproduction assumptions, not calibrated hardware truths. $T$ sums the four estimated costs; it does not terminate the process.
...
Orchestrator & Single-node K3s/Kubernetes v1.36.2+k3s1~\cite{k3s2026}; control-plane Ready \\
...
Makespan also improved in all five pairs, but by a smaller and more variable amount: 0.55\% to 5.87\%, with a mean paired improvement of 2.47\%. Its exploratory interval crosses zero. An exact two-sided sign test for five same-direction pairs gives $p=2/2^5=0.0625$. The observed direction is encouraging, but five runs on a shared node do not support a general throughput claim.
...
\subsection{Projected 200-workload scaling scenario}

To make the intended large-burst hypothesis explicit before an expensive cluster run, we generated 200 deterministic heavy-training proxy workloads using seed 20260810 and the same feature ranges as the pilot. The proxy retains a four-core node model, and its global time unit is calibrated to the measured mean baseline JCT from the five-pair pilot. Policy effects are not fitted to those five measurements. Instead, this scenario deliberately assumes that the complete \system{} policy improves on duration-only ordering by an additional 6\% in average JCT, 3\% in p95 JCT, and 2\% in makespan. These assumptions are inputs to a what-if analysis, not observations.

Under this optimistic scenario, average JCT is projected to fall from 990.8~s under the Kubernetes-default proxy to 693.6~s under \system{}, corresponding to a 30.0\% reduction. Projected p95 JCT falls from 1296.6 to 1118.6~s (13.7\%), and projected makespan falls from 1304.5 to 1137.6~s (12.8\%). Duration-only is projected to improve the same metrics by 25.5\%, 11.1\%, and 11.0\%, respectively. Table~\ref{tab:projection} and Fig.~\ref{fig:projection} report the complete declared scenario.

\begin{table}[t]
\caption{Projected 200-Workload Metrics (seconds)}
...
\begin{tabular}{lrrr}
\toprule
Policy & Avg. JCT & p95 JCT & Makespan \\
\midrule
Kubernetes default & 990.8 s & 1296.6 s & 1304.5 s \\
Duration-only & 737.9 s & 1153.2 s & 1160.8 s \\
\system{} what-if & 693.6 s & 1118.6 s & 1137.6 s \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.80\columnwidth]{figures/fig_optimistic_improvements.pdf}
  \caption{Projected improvement over the Kubernetes-default proxy (positive is better). Values assume 6\%/3\%/2\% gains beyond duration-only for average JCT/p95 JCT/makespan; they are synthetic, not executed measurements.}
  \label{fig:projection}
\end{figure}

\subsection{Threats to validity}

The main threat is the environment. The evaluation node was an operational server, not an isolated benchmark host. Alternating arm order, paired inputs, cool-node checks, and unchanged protected-service state reduce systematic bias, but cannot remove background variation. We therefore label this run as a production-path pilot rather than an isolated benchmark.
...
Finally, the measured treatment combines six-feature ranking, queue prefill, and bounded tail balancing. No live duration-only, reversed-order, FastPath-off, or no-tail arm was included. The pilot therefore measures the deployed policy as a whole and cannot assign causality to one component. The artifact now contains tested controller support and a preregistered 30-block, six-arm protocol for those comparisons, but that matrix has not been executed and contributes no result to this paper. These limitations are part of the result boundary, not future details that can be assumed away.

The 200-workload projection has a stricter boundary: it is not a measurement, confidence interval, or statistical extrapolation from 200 cluster runs. Its additional advantage over duration-only is an explicit optimistic assumption. It is included to preregister a scaling hypothesis and expected visualization; only a complete live matrix can confirm, reject, or replace its values.
...
An explicitly optimistic 200-workload scenario projects reductions of 30.0\% in average JCT, 13.7\% in p95 JCT, and 12.8\% in makespan relative to a Kubernetes-default proxy. These are hypothesis values, not final performance claims; they must be replaced by the registered live comparison before they can support a scaling conclusion.

The result is promising but preliminary. The next evaluation should execute the registered matrix on a dedicated cluster and then add real framework workloads. GPU and multi-node experiments must give $G$ and $P$ measured communication and parallelism semantics; profiling should replace generator-provided estimates; and aging or an explicit tail objective should improve robustness. Source code, the five-pair evidence, structural diagnostics, and the unexecuted ablation protocol are available in the project artifact~\cite{kubeml2026artifact}.

\section*{Author Contributions}
Amir Mirzaei Movahed: conceptualization, software, experiment execution, data curation, visualization, and writing--original draft. Majid Bafandeh: supervision, methodology review, validation, and writing--review and editing. Both authors reviewed and approved the manuscript.
