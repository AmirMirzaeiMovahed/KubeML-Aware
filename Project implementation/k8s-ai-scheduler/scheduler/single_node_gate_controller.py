"""Single-Node Scheduling Gate Controller for Minikube reproduction.

Simplified version of SchedulingGateController for single-node clusters.
Uses RealClusterFeedback (single node) instead of ClusterMetricsFeedback.
"""
...
class SingleNodeGateController:
    """Gate controller for single-node clusters (Minikube).

    Simplified version of SchedulingGateController that assumes a single
    target node. Uses RealClusterFeedback for adaptive pacing instead of
    ...
    def __init__(
        ...
        *,
        ...
        self.logger = logger or JsonEventLogger(
            static_fields={
                "component": "single-node-gate-controller",
    ...
    def _record_metadata(self, settings: RunSettings) -> Dict[str, object]:
        return {
            "profile": "single-node-scheduling-gate",
        ...
        try:
            for index, (job, record) in enumerate(zip(ordered, records, strict=True)):
                ...
                self.metrics.inc(
                    ...
                    labels={"profile": "single-node-scheduling-gate"},
                )
    ...
    def run_forever(self) -> None:
        self._start_health()
        self.logger.info(
            "controller_ready", gate_name=self.gate_name, target_node=self.target_node
        )
        try:
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--target-node", default="minikube")
    ...
    parser.add_argument(
        "--results", default="../results/single_node_gate_schedule_run.json"
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        controller = SingleNodeGateController(
            config, gate_name=args.gate_name, target_node=args.target_node
        )
